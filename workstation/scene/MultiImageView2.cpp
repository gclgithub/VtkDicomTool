...
void MultiImageView2::initiate()
{
	...
	for (int i = 0; i < 4; i++) 
	{
		...
		{
			scene->set_cross_line(m_cross_line);
...
void MultiImageView2::set_img_data(SPTR<vtkImageData> img_data)
{
	m_img_data = img_data;
	...
	for (int i = 0; i < 4; i++)
	{
		if (i != ViewType::VOL_M)
		{
			m_scene_map[ViewType(i)]->load_data(m_img_data, mat);
		}
	}
	qvtk_widget_->renderWindow()->Render();
}
