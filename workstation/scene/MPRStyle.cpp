...
void MPRStyle::OnMouseMove()
{
	...
	if (auto render_scene = GetCurRenderScene())
	{
		...
		auto center_y = res->get_center()[1];
		auto spacing_y = res->get_spacing()[1];

		render_scene->updateInfoPosition(world_pos[0], center_y * 2 - world_pos[1] - spacing_y, world_pos[2]);
		//std::cout << "pos:" << pos[0] << "," << pos[1] << ",world_pos:" << world_pos[0] << "," << world_pos[1] << "," << world_pos[2] << std::endl;
		//if (auto tools = Data_Info.get_measure_tool().lock()) MouseFunction = MEASURE;

		//update roi
		if (render_scene->get_viewType() == TRA_M)
		{
			int cur_index, slice_count;
			res->get_slicer_num(slice_count, cur_index);

			//auto roi_list = Data_Info.get_roi_list().lock();
			//for (int i = 0; i < roi_list->size(); i++)
			//{
			//	auto roi = roi_list->at(i);
			//	roi->update_contour(cur_index, this->GetInteractor());
			//}
		}
...
void MPRStyle::moveReslicer(PTR<ViewRenderScene> ren_scene, int slicer)
{
	...
	ren_scene->updateInfoPosition(world_pos[0], center_y * 2 - world_pos[1] - spacing_y, world_pos[2]);
	...
	//update roi
	if (ren_scene->get_viewType() == TRA_M)
	{
		int cur_index, slice_count;
		res->get_slicer_num(slice_count, cur_index);
